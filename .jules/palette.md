...
## 2026-07-02 - Inline Filter Toggle Context & Dead-Ends
**Learning:** When users click inline interactive elements like keyword tags or author names within a list item (e.g. `PackageCard`) to apply a filter, failing to provide an obvious "toggle off" mechanism or visual active state turns these elements into dead-ends. Users expect to click an active tag again to remove the filter, but without visual feedback or modified interaction handlers, they are forced to hunt for a separate "Clear filter" button elsewhere in the UI.
**Action:** When using list item elements as inline filters, dynamically check if they match the active filter state. If active, apply distinct visual styling (e.g. `aria-pressed="true"`, darker backgrounds), update their `aria-label` to "Clear filter", and modify the `onClick` handler to reset the filter value rather than re-applying it.
